...
void ship::Fire(){
    ...
    if ((GetTickCount() - lastShotTime) >= shotCooldown)
    {
        ...
        if (shotsUsed < maxShots)
        {
            ...
            // Makes new Shot
            Shot shot( "Boom... ", 1., 3, Color::Black, resourcePath() + "spaceMissiles_040.png", 20, 35, 3e3, GetWindowSizeX(), GetWindowSizeY(), 2e2, x, y, phy + 180);
...
void updateLife(int *p, int *e, Text *et, Text *ee)
{
    et->setString("hp: " + to_string(*p));
    ee->setString("hp: " + to_string(*e));
}

int main(){
    
    int playerhp, player2hp = 100;
    ...
    ship p("player1", gravity, playerhp, Color::Green, resourcePath() + "spaceShips_003.png", sizeX, sizeY, maxShots, windowSizeX, windowSizeY, vmax, x, y, phy);
    ...
    ship2 p2("player2", gravity, player2hp, Color::Green, resourcePath() + "spaceShips_008.png", sizeX, sizeY, maxShots, windowSizeX, windowSizeY, vmax, x, y, phy);
    ...
    playerlife.setString("Joueur 1 : ");
    playerlife.setCharacterSize(50);
    playerlife.setFillColor(Color::Red);
    playerlife.setStyle(Text::Style::Bold);
    playerlife.setPosition(10, 0);
    
    Text player2life;
    player2life.setFont(font);
    player2life.setString(": Joueur 2" );
    player2life.setCharacterSize(50);
    player2life.setFillColor(Color::Red);
    player2life.setStyle(Text::Style::Bold);
    player2life.setPosition(970, 0);
    
    Text PlayerVictory;
    PlayerVictory.setFont(font);
    PlayerVictory.setCharacterSize(200);
    ...
    while (window.isOpen()){
        ...
        updateLife(&playerhp, &player2hp, &playerlife, &player2life);
        ...
        if ( !p2.isDead() ){
            ...
            for (int i = 0 ; i < (p.ShotsInSpace).size(); i++){
                ...
                (p.ShotsInSpace[i]).texture.loadFromFile(resourcePath() + "spaceMissiles_040.png");
                ...
                else if (  ( (p.ShotsInSpace)[i].shape.getGlobalBounds().intersects( p2.shape.getGlobalBounds() ) )) player2hp -= 10;
            }
        ...
        if ( !p2.isDead() ){
            ...
            for (int i = 0 ; i < (p2.ShotsInSpace).size(); i++){
                ...
                (p2.ShotsInSpace[i]).texture.loadFromFile(resourcePath() + "spaceMissiles_040.png");
                ...
                else if ( !p.isDead()  && ( (p2.ShotsInSpace)[i].shape.getGlobalBounds().intersects( p.shape.getGlobalBounds() ) )) playerhp -= 10;
        ...
        for (int i = 0 ; i < (PlanetsInSpace).size(); i++ ){
            ...
            {
                PlanetsInSpace[i].texture.loadFromFile(resourcePath() + "spaceMeteors_001.png");
            ...
            {
                PlanetsInSpace[i].texture.loadFromFile(resourcePath() + "spaceMeteors_002.png");
                PlanetsInSpace[i].texture.setSmooth(true);
                PlanetsInSpace[i].shape.setTexture(PlanetsInSpace[i].texture);
            }
            else if (25 < PlanetsInSpace[i].life && PlanetsInSpace[i].life <= 50)
            {
                PlanetsInSpace[i].texture.loadFromFile(resourcePath() + "spaceMeteors_003.png");
                PlanetsInSpace[i].texture.setSmooth(true);
                PlanetsInSpace[i].shape.setTexture(PlanetsInSpace[i].texture);
            }
            else if (PlanetsInSpace[i].life <= 25)
            {
                PlanetsInSpace[i].texture.loadFromFile(resourcePath() + "spaceMeteors_004.png");
                PlanetsInSpace[i].texture.setSmooth(true);
                PlanetsInSpace[i].shape.setTexture(PlanetsInSpace[i].texture);
            }
